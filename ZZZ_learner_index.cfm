...
<cfsilent>
	...
	<cfif SESSION.ACCESS_VIRTUALCLASS eq "1">
		<cfset get_next_lesson_virtual = obj_tp_get.oget_next_lesson(u_id="#u_id#",m_id="10",subscribed="yes")>
	...
	<div class="main-panel">
		...
		<div class="content">
			...
			<!--- <cfif SESSION.LANG_CODE eq "fr">
				...
				Un incident est en cours chez notre opérateur téléphonique, notre standard est indisponible actuellement. 
				<br>
				Vous pouvez nous joindre par téléphone sur les numéros suivants :
				<br>
				<strong>07 57 81 95 24</strong>
				<br>
				<strong>07 80 91 74 97</strong>
				</div>

			<cfelseif SESSION.LANG_CODE eq "en">
				<div class="alert alert-danger" role="alert" align="center">
					An incident is underway with our phone operator, our switchboard is currently unavailable.
					<br>
					You can reach us by phone on the following numbers:
					<br>
					<strong>+33 (0)7 57 81 95 24</strong>
					<br>
					<strong>+33 (0)7 80 91 74 97</strong>
				</div>

			<cfelseif SESSION.LANG_CODE eq "de">
				<div class="alert alert-danger" role="alert" align="center">
					Bei unserem Telefonanbieter ist ein Zwischenfall im Gange, unsere Telefonzentrale ist derzeit nicht erreichbar.
					<br>
					Sie erreichen uns telefonisch unter der folgenden Nummer :
					<br>
					<strong>+49 (0)7 57 81 52 23</strong>
				</div>
			</cfif> --->
			...
			<div class="col-lg-4 col-md-6 mb-3">
				...
				<div class="card h-100 <cfif SESSION.ACCESS_NEXT eq "1">border<cfelse>bg-light</cfif>">
					<div class="card-body">
						...
						<div class="w-100">
							<h5 style="font-size:20px" class="d-inline <!---<cfif SESSION.ACCESS_NEXT eq "1">text-red</cfif>--->"><i class="fa-thin fa-rocket-launch <!---<cfif SESSION.ACCESS_NEXT eq "1">text-red</cfif>---> fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_next_activity')#</cfoutput></h5>
						...
						<cfif SESSION.ACCESS_NEXT eq "1">
							...
							<cfloop list="#method_list#" index="cor">
								...
								<cfif isdefined("get_next_lesson_#cor#") AND evaluate("get_next_lesson_#cor#").recordcount neq "0">
									...
									<!--- <cfdump var="#evaluate("get_next_lesson_#cor#")#"> --->
			...
			<div class="col-lg-4 col-md-6 mb-3">
				<div class="card h-100 <cfif evaluate("SESSION.ACCESS_#ucase(cor)#") eq "1">border<cfelse>bg-light</cfif>">
					<div class="card-body pb-0">
						...
						<div class="w-100">
							<h5 class="d-inline" style="font-size:20px"><i class="fa-thin <cfoutput>#card_icon#</cfoutput> <!---<cfif evaluate("SESSION.ACCESS_#ucase(cor)#") eq "1">text-red</cfif>---> fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_#cor#')#</cfoutput></strong></h5>
							...
							<cfif tp_status_id neq "3" AND tp_status_id neq "">
								...
								<div class="card border bg-light m-0 mt-3">
									<div class="card-body">
										...
										<div align="right" class="clearfix w-100">
											
<cfif SESSION.USER_PROFILE eq "TEST" AND SESSION.USER_STATUS_ID eq "3">
	<a class="btn btn-red m-0 mt-2" href="learner_launch_1.cfm?trigger_launch=1">ONBOARDING !</a>
<cfelse>
	<a class="btn btn-outline-red m-0 mt-2" href="./common_tp_details.cfm?t_id=#tp_id#">#ucase(obj_translater.get_translate("btn_book_short"))#</a>
</cfif>
			...
			<div class="col-lg-4 col-md-6 mb-3">
				...
				<div class="card h-100 <cfif SESSION.ACCESS_VIRTUALCLASS eq "1">border<cfelse>bg-light</cfif>">
					<div class="card-body">
						...
						<div class="w-100">
							<h5 style="font-size:20px" class="d-inline <!---<cfif SESSION.ACCESS_VIRTUALCLASS eq "1">text-red</cfif>--->"><i class="fa-thin fa-users-viewfinder <!---<cfif SESSION.ACCESS_VIRTUALCLASS eq "1">text-red</cfif>---> fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_virtual')#</cfoutput></h5>
					...
					<cfif SESSION.ACCESS_VIRTUALCLASS eq "0">
						<div class="card-footer pt-0" align="right">
							<a class="btn btn-outline-red btn_discover_virtualclass m-0 mt-2"><cfoutput>#ucase(obj_translater.get_translate('btn_more_info'))#</cfoutput></a>
						</div>
					<cfelse>
						<div class="card-footer pt-0" align="right">
							<a class="btn btn-outline-red m-0 mt-2" href="./learner_virtual.cfm"><cfoutput>#ucase(obj_translater.get_translate('vc_btn_manage_tp'))#</cfoutput></a>
						</div>
					</cfif>
			...
			<div class="col-lg-4 col-md-6 mb-3">
				...
				<div class="card h-100 <cfif SESSION.ACCESS_EL eq "1">border<cfelse>bg-light</cfif>">
					<div class="card-body">
						...
						<div class="w-100">
							<h5 style="font-size:20px" class="d-inline"><i class="fa-thin fa-phone-laptop <!---<cfif SESSION.ACCESS_EL eq "1">text-red</cfif>---> fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_elearning')#</cfoutput></h5>
								...
								<div class="card border bg-light m-0 mt-3">
									<div class="card-body">
										...
										<cfif formation_id eq "3">
											<a class="btn btn-outline-red m-0 mt-2 pull-right" href="common_practice.cfm?f_id=3">#ucase(obj_translater.get_translate("table_th_access"))#</a>
										<cfelse>	
											<a class="btn btn-outline-red m-0 mt-2 pull-right" href="el_dashboard.cfm">#ucase(obj_translater.get_translate("table_th_access"))#</a>
										</cfif>
					...
					<cfif SESSION.ACCESS_EL eq "0">
						<!--- <div class="card-footer pt-0" align="right">
							<a class="btn btn-secondary btn_discover_el m-0 mt-2"><cfoutput>#obj_translater.get_translate('btn_more_info')#</cfoutput></a>
						</div> --->
					<cfelse>
						<!--- <div class="card-footer pt-0" align="right">
							<!--- <cfif SESSION.USER_ID eq "411">
								<a class="btn btn-outline-red m-0 mt-2" href="_AD_elearning_index.cfm"><cfoutput>Démarrer</cfoutput></a>
							<cfelse>
								<cfif SESSION.USER_ID eq "12177" OR SESSION.USER_ID eq "12176" OR SESSION.USER_ID eq "9966" OR SESSION.USER_ID eq "14409" OR SESSION.USER_ID eq "32830">
									

									<a class="btn btn-outline-red m-0 mt-2" href="el_dashboard.cfm"><cfoutput>#ucase(obj_translater.get_translate("table_th_access"))#</cfoutput></a>
							

								<cfelse>

									<a class="btn btn-outline-red m-0 mt-2" href="common_practice.cfm"><cfoutput>#ucase(obj_translater.get_translate("table_th_access"))#</cfoutput></a>
			...
			<div class="col-lg-4 col-md-6 mb-3">
				<div class="card h-100 border">
					<div class="card-body">
						...
						<div class="w-100">
							<h5 style="font-size:20px" class="d-inline"><i class="fa-thin fa-address-card fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_account')#</cfoutput></h5>
			...
			<div class="col-lg-4 col-md-6 mb-3">
				...
				<div class="card h-100 <cfif SESSION.ACCESS_CERTIF eq "1">border<cfelse>bg-light</cfif>">
					<div class="card-body">
						...
						<div class="w-100">
							<h5 style="font-size:20px" class="d-inline"><i class="fa-thin fa-file-certificate <cfif SESSION.ACCESS_CERTIF eq "1">text-secondary</cfif> fa-lg mr-1"></i> <cfoutput>#obj_translater.get_translate('card_title_certif')#</cfoutput></h5>
							...
							<div class="card border bg-light m-0 mt-3">
								<div class="card-body">
									...
									<div class="d-flex">
										...
										<!--- <div align="right" class="clearfix"><a class="btn btn-red m-0 mt-2" href="./learner_virtual.cfm">S'ENTRAINER</a></div> --->
					...
					<!--- <cfif SESSION.ACCESS_CERTIF eq "0">
						<div class="card-footer pt-0" align="right">
...
<script>
	...
	// <cfif not isdefined("SESSION.ALERT_WELCOME") AND listFindNoCase("LEARNER", SESSION.USER_PROFILE)>

	// 	$('#window_item_lg').modal({keyboard: false,backdrop:'static'});
	// 	$('#modal_title_lg').text("<!---<cfoutput>#obj_translater.get_translate('title_page_trainer_index')#</cfoutput>--->*WEFIT LMS*");		
...
// alert(<cfoutput>#SESSION.USER_STATUS_ID#</cfoutput>);
	...
	$('.btn_view_quiz').click(function(event) {	
		...
		$('#modal_body_xl').load("<cfoutput>modal_window_quiz.cfm?quiz_user_id="+quiz_user_id+"&u_id=#SESSION.USER_ID#</cfoutput>", function() {});
	})
	...
	$('.btn_launch_lesson').click(function(event) {	
		...
		<cfif SESSION.USER_ID eq "29768">
		$('#window_item_lg').modal({backdrop: 'static', keyboard: true});
		$('#modal_title_lg').text("<cfoutput>#obj_translater.get_translate('js_modal_title_nextlesson')#</cfoutput>");
		$('#modal_body_lg').load("_AD_modal_window_lesson_launch.cfm?l_id="+idtemp, function() {});
		<cfelse>
	...
	$('.btn_join_class').click(function(event) {
        ...
        $.ajax({				 
			...
			type: 'POST',
			data: {tp_id: event.target.dataset.tid, l_id: event.target.dataset.lid},
