...
<cfset get_lesson = obj_query.oget_lessons(st_id="5,6",t_id="#t_id#",ghosted="1",orderby="ASC")>
		...
		<td valign="top">
			<table bgcolor="#ECECEC" style="font-family:Arial, Helvetica, sans-serif; font-size:10px;" width="100%" cellpadding="6" cellspacing="1">
				...
				<tr>
					...
					<td width="30%" bgcolor="##FFFFFF">#get_lesson.account_name#</td>
				...
				<tr bgcolor="##FFFFFF">
					...
					<td align="center">
						<cfquery name="get_" datasource="#SESSION.BDDSOURCE#" result="data">
							SELECT la.*, u.user_firstname, u.user_name, u.user_id
							FROM lms_lesson2_attendance la
							LEFT JOIN user u ON u.user_id = la.user_id
							WHERE la.lesson_id = <cfqueryparam cfsqltype="cf_sql_integer" value="#lesson_id#">
							AND la.lesson_signed = 1
						</cfquery>
						<cfloop query="#get_#">
							#get_.user_firstname#
							#obj_lms.get_learner_signature(l_id="#lesson_id#", u_id="#user_id#", size="80")#
							<br/>
						</cfloop>
					</td>					
					<td align="center">
