...
<cfelse>
	<cfset get_lesson_tech = obj_query.oget_lesson_default_tech(u_id="#p_id#")>
...
<cfif get_lesson.method_id neq "7">
	...
	<cfform id="launch_form" action="gateway.cfm" target="_blank">
	<cfoutput>
	...
	<cfif get_lesson.status_id eq "1" OR get_lesson.status_id eq "2">
		...
		<div align="center">
			...
			<!---------------------- VIRTUAL CLASS ----------------------------->
			<cfif get_lesson.method_id eq "10">
				<cfif now() gte dateadd("n",-10,get_lesson.lesson_start) AND now() lte dateadd("n",+5,get_lesson.lesson_start)>				
					<input type="submit" class="btn btn-red" value="#obj_translater.get_translate('btn_launch_lesson')#">
				<cfelse>
					<cfset rule_launch = "1">
					<input type="submit" class="btn btn-red disabled" value="#obj_translater.get_translate('btn_launch_lesson')#*">
				</cfif>
		...
		<cfif isdefined("rule_launch")>
			<cfif get_lesson.method_id eq "10">
				#obj_translater.get_translate_complex('course_rule_launch_vc')#
			<cfelse>
...
<cfelse>
	...
	$(document).ready(function() {
		...
		$("#launch_form").submit(function( event ) {
			...
			if( $sigdiv.jSignature('getData', 'native').length == 0 && <cfoutput>#get_lesson.provider_id#</cfoutput> !== 2 ) {
				alert('<cfoutput>#obj_translater.get_translate('js_warning_signature')#</cfoutput>');
				return false;
			}
			else
